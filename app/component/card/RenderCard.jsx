...
const RenderCard = (props) => {
  ...
  return (
    <View style={styles.item}  onLayout={(event) => setWidthOfView(event.nativeEvent.layout.width)}>
      {/* Header */}
      <View
        style={{
          display: "flex",
        ...
        {props.item.image.length > 0 &&
          <RoomImageCarousel image={props.item.image} width={widthOfView - 50} height={300} paginationColor={"#84E6E3" } />
       }
           ...
           props.item.comments.map((item, index) => (
              <View key={index}>
                ...
                <View style={{ display: "flex", alignItems: "flex-end" }}>
                  <Text category="label" style={{ marginRight: 10,color:"#A4A2A2"}}>
                    {item.date}
                  </Text>
                </View>
              </View>
